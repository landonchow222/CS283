...
int get_student(int fd, int id, student_t *s){
    ...
    // Check if the record is empty (student not found)
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    // Calculate the offset for the student's record
    off_t offset = (id - 1) * sizeof(student_t);
    ...
    if (bytes_read == sizeof(student_t) && 
        memcmp(&existing_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0) {
        printf(M_ERR_DB_ADD_DUP, id);
        return ERR_DB_OP;
    }

    // Seek back to write the new student record
    if (lseek(fd, offset, SEEK_SET) == -1) {
    ...
    if (id == 99999) {
        if (lseek(fd, offset + sizeof(student_t), SEEK_SET) == -1) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }

        student_t empty_record = {0};
        if (write(fd, &empty_record, sizeof(student_t)) != sizeof(student_t)) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }
    }
...
int count_db_records(int fd){
    ...
    if (lseek(fd, 0, SEEK_SET) == -1) {
        printf(M_ERR_DB_READ);  // Handle the error directly
        return ERR_DB_FILE;
    }
    ...
    // Check if the read failed (read returns -1 on error)
...
int print_db(int fd){
    ...
    student_t student;
    int record_found = 0;  // Flag to check if at least one valid record exists
    ...
    // Read through the entire database
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
...
int compress_db(int fd){
    ...
    // Reopen the original database file for reading
    ...
    // Read through the original database and copy only valid records
    while (read(fd, &student, sizeof(student_t)) == sizeof(student_t)) {
